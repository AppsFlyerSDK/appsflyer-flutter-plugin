...
| Generated `appsflyer_sdk-Swift.h` | Exposes the `AppsFlyerAttribution` singleton interface to Objective-C: the three `handleOpenUrl:`/`continueUserActivity:` entry points, all pinned with explicit `@objc(...)` selectors in the Swift source. Bridge readiness is opened only from `AppsflyerSdkPlugin` after `init()`; it is not a public `@objc` method. |
...
- **`handleLaunchOptions` is forwarded at launch**: `application:didFinishLaunchingWithOptions:` sanitizes launch options and immediately sends `handleLaunchOptions` through `AFRPCBridge` (fire-and-forget). Serialization and RPC failures are logged via `os_log`; there is still no host callback. The native SDK has no `initialize` dependency on that call; it only sets a pending-deeplink flag that `registerSessionReadyListener` samples later. `NSURL` values are converted to strings and non-JSON values are dropped. This JSON-RPC hop is interim until AppsFlyerRPC exposes a public typed lifecycle entry point on the bridge.
