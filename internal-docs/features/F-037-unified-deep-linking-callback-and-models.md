...
- **`DeepLink.isDeferred` is populated on iOS as well as Android.** An earlier revision of this document stated that the iOS `clickEvent` dictionary never carries an `is_deferred` key and that `isDeferred` therefore always returned `null` on iOS; that is no longer true and required no change in the Flutter plugin. Measured on 2026-08-27 against `AppsFlyerRPC` 7.0.13 on an iPhone 17 Pro simulator (iOS 26.5): a deferred resolution (browser click, install, launch with no URL) returned `is_deferred: true` with `match_type: probabilistic`, a direct Universal Link into the running app returned `is_deferred: false`, and a direct custom-scheme open returned `is_deferred: false`. So the flag arrives through both iOS entry points (`continueUserActivity` and `handleOpenUrl`) and carries a correct value, not a constant. **Confirmed on a physical device on 2026-08-27** (iPhone, profile build, `https://af-flutter-plugin.onelink.me/SBnY/flutter-qa-link`): after a manual Safari click, install and a launch with no URL, `onDeepLinking` reported `Status.FOUND` with `is_deferred: true`, `match_type: probabilistic` and the server-resolved `deep_link_value` / `deep_link_sub1`, while `is_first_launch: true` proved the launch was the first after install. Note that the same run reported `af_status: Organic` in `onInstallConversionData` — a probabilistic match is enough to resolve the link but not to attribute the install, so the two callbacks legitimately disagree.
