...
Every callback on this path calls `AppsFlyerLib` directly rather than crossing the RPC bridge. These are native-to-native hops that no Dart call originates, and a JSON envelope loses exactly what the SDK needs: `handleLaunchOptions:` reads only the live `NSUserActivity` inside the launch-options dictionary, and the RPC `continueUserActivity` handler could only rebuild a synthetic activity from `activityType` plus `webpageURL`. Because of this the Core podspec declares `AppsFlyerFramework` explicitly alongside `AppsFlyerRPC`; the SPM manifest already listed the `AppsFlyerLib` product. The React Native plugin forwards the same callbacks the same way, and these call sites move to a typed lifecycle API once AppsFlyerRPC exposes one.

- URL-scheme links map to `handleOpen(_:options:)`, Universal Links to `continue(_:restorationHandler:)`. The deprecated iOS 8 `sourceApplication`/`annotation` shape is not forwarded: UIKit stops calling that selector once `application:openURL:options:` exists, which `FlutterAppDelegate` provides, so it is unreachable on the iOS 13 floor.
- launch options are forwarded at cold start straight from `application:didFinishLaunchingWithOptions:`, with no readiness gate: the call has no `initialize` dependency and only sets the pending-deeplink flag that `registerSessionReadyListener` samples later.
- `AppsFlyerAttribution` queues early URL/Universal Link entries — the original `URL` plus options, or the original `NSUserActivity` — and replays them after plugin-internal `markBridgeReady(markedBy:)`, because the SDK skips OneLink resolution while `devKey`/`appId` are still unset. Queue state is serialized on the main queue inside the singleton. Nothing is logged on this path because the SDK calls report nothing actionable; the host app learns deep-link outcomes only through `af-events` (F-037). `markBridgeReady` records the owning plugin instance so `resetBridgeStateIfOwned(by:)` on engine detach clears `isBridgeReady` / `pendingRequests` without affecting a live second engine. A parameterless `@objc markBridgeReady` is intentionally not exposed: it would open the gate without an owner and break detach cleanup.

These lifecycle calls are implementation details and are not public Dart methods.
...
- On Android, `AppsFlyerPurchaseConnector` keys its `MethodChannel`, `ConnectorWrapper`, and validation listeners per `FlutterPluginBinding`, so add-to-app / multi-engine setups do not share one channel or tear down another engine's connector on detach. `attachmentsLock` guards only map mutations; `EngineAttachment.dispose()` (including `stopObservingTransactions`) runs outside the lock on both attach and detach so a slow billing teardown cannot block lifecycle for other engines.
- On iOS, `PurchaseConnectorPlugin` remains a process singleton holding one channel, so the last engine to register owns it — per-engine connector state is not available here because the native `PurchaseConnector.shared()` is process-scoped, unlike Android's per-engine `PurchaseClient`. `register(with:)` clears the previous channel's handler before installing the new one, so an engine that lost ownership cannot keep driving `configure` and does not leave a live handler behind that its own ownership-guarded teardown would skip. It publishes no instance of its own and therefore receives no detach callback directly: `AppsflyerSdkPlugin.detachFromEngineForRegistrar:` calls `tearDownForEngineDetach(registrar:)`, which stops transaction observation, clears the revenue delegate and the channel, and lets the next engine call `configure` again. The teardown is skipped unless the detaching registrar still owns the channel, so a stale engine cannot stop observation for a live one.
...
| iOS attribution adapter | `ios/appsflyer_sdk/Sources/appsflyer_sdk/AppsFlyerAttribution.swift` | Queues early URL/Universal Link callbacks and forwards them to `AppsFlyerLib` |
