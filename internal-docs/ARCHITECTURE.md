...
- Dart generally preserves `null` in the `params` map. Each typed native request decides whether null means “absent,” a real nullable value, or invalid input. Do not assume that omission and explicit null are interchangeable; for example, nullable consent values and clearing the iOS sharing filter intentionally use null.
- Android converts the Flutter map to `JSONObject` and its parser uses typed `opt*` helpers. Some missing, null, or wrong-typed optional values therefore collapse to parser defaults.
