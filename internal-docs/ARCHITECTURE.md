...
- Android detaches channels, clears pending events, shuts down its executor, and releases its RPC handler/context when the Flutter engine detaches. In-flight blocking-RPC work that completes after shutdown is not delivered back to Dart (`PLUGIN_DETACHED`).
