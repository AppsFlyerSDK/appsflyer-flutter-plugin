...
- Event buffering is in memory and never persisted, so it does not survive process death. Both platforms cap the buffer at 64 events and drop the oldest on overflow; the buffer is process-scoped on Android and engine-scoped on iOS. Malformed events are dropped. On Android, `AppsFlyerEventBus` distinguishes three sink outcomes: deliver (`DELIVERED`), keep the head event for the next attach (`RETRY_LATER`, including while `isEngineDetached` is set during teardown), and drop a refused head event (`DROP`) so the rest of the buffer can continue. Dart attaches its single `af-events` subscription on the first `register*Listener` call, and that attach flushes the whole native buffer at once: `_AppsFlyerListenerRegistry` holds any replayed event whose listener has not registered yet — capped at 64, the same bound as the native buffers — and delivers it when that listener registers. An event arriving after its listener has been registered and then unregistered is logged and dropped.
...
| Android event relay | `android/src/main/kotlin/com/appsflyer/appsflyersdk/AppsFlyerEventBus.kt` | Process-scoped event buffering, FIFO replay, `EventSendResult` delivery (deliver / retry / drop), and sink attach/detach across engine recreation |
